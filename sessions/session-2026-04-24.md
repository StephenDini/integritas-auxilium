...
```
  ...
  |   20x20 room       |
  |                    |
  +------------------[S]   <- secret door entry (SE corner)
           |
        20 ft north
           |
[W]+----------------+      <- top west door (near NW corner)
   | (§)  (§)  (§) |      <- ladders (left, center, right)
   | [p] *goo* [xx] |
   | *goo*  *goo*   [E]
   | [xx] *goo* [p] |
   | [xx] *goo*     |      <- all 3 beetles dead
[W]+----------------+      <- bottom west door (near SW corner) — hallway here
           |
     hallway (south, descends 40 ft)
```
