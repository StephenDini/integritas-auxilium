# Fraxiga

**Class:** Wood Elf
**Level:** 7
**XP:** 47,295+ (shared party pool; reached level 7 during session ~14)

## Description

Wood Elf; a druid-type caster and archer. Name spelling is uncertain across sources — also seen as "Forxiga" (and the DM slipped and called her "Ezekiel" once in session ~14). Can **shift into rat form** (and does a small rat "roar" as a party trick).

## Current Notable Items (through session ~14)

- **Enchanted chainmail** (Session 1)
- **Magic bow** — "bow of acceleration": once/day roll 1d4 for that many rounds of double attacks; +1 bow (attribution to Fraxiga likely but confirm)
- Enchanted short bow — ownership still unresolved between Fraxiga and Burt
- 2 healing potions (Session 1)
- Druid-type spells seen in play: Stone Shape, Heat Metal, Faerie Fire, Cure Light Wounds, Detect Magic, Spider Climb (used to carry the blink dog down a ladder, session ~14)

## Companion

- **Blink dog (deceased)** — Fraxiga's animal companion, killed in session ~14 by a Fireball that erupted from Bruce's Wand of Wonder. Remains bagged in case of future raising. A replacement companion is wanted.

## Notes

- Found the secret passages in the Castleton cliffside manor in Session 1.
- Further details to be filled from Discord archive.
